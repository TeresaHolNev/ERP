...
export class CrearProvComponent implements OnInit {
  @ViewChild('cif') cifRef : ElementRef;    //Hacer una selección del DOM, en el input
  ...
  crearProv(){
    this.mostrarAlerta = false;//Para que se reinicie al crear un proveedor
    this.enviando = true;
      this.proveedor = this.guardarProv();
      this.proveedoresService.postProveedor(this.proveedor)   //Declaramos el servicio
          .subscribe((resp:any)=>{
            this.router.navigate(['/listado-proveedores']);//cuando haya exito redireccione a la lista de los proveedores
            this.enviando = false;
          }, (error:any)=>{
            ...
            if(error.error.errores.errors.cif.message){
              ...
              this.cifRef.nativeElement.focus();//mueve el foco al campo cif cuando exista error
            }
          });
  }
